...
from inputUS.models import KeywordGlossary, Result, Similarity_Analysis, UserStory_element, UserStory_Who, UserStory_What, UserStory_Why, US_Upload
...
def well_formed_an(obj_id):
    ...
    if get_UserStory_data:
        ...
        if get_UserStory_data.Who_full and get_UserStory_data.Who_full.Who_full == '':
            ...
            if get_UserStory_data.Who_full.Who_full != '' and get_UserStory_data.What_full.What_full != '':
                if get_UserStory_data.UserStory_Who.Who_identifier not in ('As a', 'As an', 'As') or get_UserStory_data.UserStory_What.What_identifier not in ('I want', 'I want to'):
        ...
        well_formed_result = {"index": index,
                    ...
                    for synset in synsets:
                        ...
                        if synset_class:
                            ...
                            for keyword_word in keyword_words:
                                if keyword_word not in keyword_to_sentence_class:
                                    keyword_to_sentence_class[keyword_word] = set()
                                keyword_to_sentence_class[keyword_word].add(synset_class)

        labels = []
        sentence_classify = {"index": well_formed_res['index'],
                             "userstory": text,
                             "role": role, 
                             "action": action,
                             "sentence_class": list(sentence_class),
                             "keyword_words": {keyword_word: list(keyword_to_sentence_class[keyword_word]) for keyword_word in keyword_words},
                             "label":"",
                            }
        ...
        for sub in dic_sub:
            if act["userstory"] == sub["userstory"]:
                if sub["cluster_label"] == -1 and act["label"] == "":
                    ...
                    recommendation = "Change the subject (role) using the standard terminology"
                    recommendation_name_user = sub["role_s_list"]
